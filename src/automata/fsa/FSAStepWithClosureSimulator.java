...
public class FSAStepWithClosureSimulator extends FSAStepByStateSimulator {
	...
	public ArrayList stepConfiguration(Configuration config) {
		...
		for (int k = 0; k < transitions.length; k++) {
			System.out.println("In initialize: " + transitions.length);
			...
			System.out.println("In initialize: " + transition.getLabel());
	...
	public boolean simulate (Configuration config){
		...
		System.out.println("Unporcessed Input:" + unprocessedInput);
		
		if(unprocessedInput.length() == 0)			
			return true;		
		else if ((configuration.getParent() == null && !currentState.getName().equals("")) || (
					!currentState.getName().equals("") && configuration.getParent() != null && 
					!configuration.getParent().getCurrentState().equals(currentState))){
			
			String label = currentState.getName();
			CharSequence c = unprocessedInput.subSequence(0, 1);
			while(label.contains(c)){
				unprocessedInput = unprocessedInput.substring(1);
				if(unprocessedInput.length() == 0)
					return true;
				c = unprocessedInput.subSequence(0,1);				
			}
		...
		for (int k = 0; k < transitions.length; k++) {
			...
			System.out.println("Unprocessed intput: " + unprocessedInput +"\n "
					+ "label: " + transLabel );
